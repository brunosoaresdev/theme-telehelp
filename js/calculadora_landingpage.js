...
jQuery(function($){
	...
	$('#accordionnovo .panel-collapse').on('shown.bs.collapse', function () {
		$(this).prev().find(".glyphicon").removeClass("glyphicon-chevron-down").addClass("glyphicon-chevron-up");
	});

	$('#accordionnovo .panel-collapse').on('hidden.bs.collapse', function () {
		$(this).prev().find(".glyphicon").removeClass("glyphicon-chevron-up").addClass("glyphicon-chevron-down");
	});

	// campo de telefone sem permissão de 0

	$('.telefone').on('input propertychange paste', function (e) {
		var reg = /^0+/gi;
	...
	//disabilita o envio do form mais de uma vez

	$('#banbotcentral button').click(function(e) {
		var btn = $(this);
	    $.post('http://jsfiddle.net/echo/jsonp/',{delay: 3}).complete(function(){
	        btn.prop('disabled', false);
	    });
	    btn.prop('disabled', true);
	});
	$('#meecentral-vendas button').click(function(e) {
		var btn = $(this);
	    $.post('http://jsfiddle.net/echo/jsonp/',{delay: 3}).complete(function(){
	        btn.prop('disabled', false);
	    });
	...
	$('#agdcentral-vendas button').click(function(e) {
		var btn = $(this);
	    $.post('http://jsfiddle.net/echo/jsonp/',{delay: 3}).complete(function(){
	        btn.prop('disabled', false);
	    });
	...
	$('#attcentral-vendas button').click(function(e) {
		var btn = $(this);
	    $.post('http://jsfiddle.net/echo/jsonp/',{delay: 3}).complete(function(){
	        btn.prop('disabled', false);
	    });
	...
	$('#ccvcentral-vendas button').click(function(e) {
		var btn = $(this);
	    $.post('http://jsfiddle.net/echo/jsonp/',{delay: 3}).complete(function(){
	        btn.prop('disabled', false);
	    });
	...
	$('#pedidos-cadastro-usuario button').click(function(e) {
		var btn = $(this);
	    //$.post('http://jsfiddle.net/echo/jsonp/',{delay: 3}).complete(function(){
	    //   btn.prop('disabled', false);
	    //});
...
function so_numero(e,args){
        ...
        var valid_chars = '1234567890'+args;
...
function sem_acento(e,args){
        ...
        //var valid_chars = '0123456789áàâãéèêíïóôõöúçñabcdefghijlmnopqrstuvxzwykÁÀÂÃÉÈÊÍÌÓÒÕÔÚÇÑABCÇDEFGHIJLMNOPQRSTUVXZWYK-_ '+args;
        var valid_chars = 'áàâãéèêíïóôõöúçñabcdefghijlmnopqrstuvxzwykÁÀÂÃÉÈÊÍÌÓÒÕÔÚÇÑABCÇDEFGHIJLMNOPQRSTUVXZWYK-_ '+args;
        var chr= String.fromCharCode(evt);
        if (valid_chars.indexOf(chr)>-1 ){return true;}
        if (valid_chars.indexOf(chr)>-1 || evt < 9){return true;}
        return false;
...
function go_step(passo){
	...
	if(passo<=1){
	   passo = 1;
       inicial = 1;
		jQuery('input#btn_passoprev').css({'visibility':'hidden', 'display':'none'});
		//jQuery('input#btn_passoprev').css('visibility','visible');
	}
    if(passo>=2){
		//jQuery('input#btn_passoprev').css('visibility','hidden');
		jQuery('input#btn_passoprev').css({'visibility':'visible', 'display':'initial'});
	}
...
function send_ce(e,f,l){
	...
	var regnome = /^[a-záàâãéèêíïóôõöúçñ ]+$/ig;
	var regn = jQuery('#'+local+'nome').val();
	if(regn.match(regnome)){
		//alert('Bom');
	}else{
		jQuery('#'+local+'botcentral').prepend('<span id="erro_cv_e" style="padding:5px;border:1px red solid;color: red;font-size: 15px;">Permitido o uso apenas de letras!</span>');
		return false;
	}
    ...
    if(local=="opo"){
        if(jQuery('#curriculo').val()==""){
        jQuery('#'+local+'botcentral').prepend('<span id="erro_cv_e" style="    margin-bottom: 10px;    width: 100%;    float: left;    text-align: center;padding:5px;border:1px white solid;color: white;font-size: 15px;">Necessário selecionar um arquivo!</span>');
        }
	...
	if(
		...
		jQuery('#'+local+'botcentral').prepend('<span id="erro_cv_e" style="padding:5px;border:1px red solid;color: red;font-size: 15px;">Obrigatório Informar um telefone válido!</span>');
		return false;
	}
	...
	if(telefone.length <10){
     ...
     if(f==40){
		...
		jQuery('#'+local+'botcentral').prepend('<span id="erro_cv_e" style="padding:5px;border:1px #155c92 solid;color: #155c92;font-size: 15px;">Processando...!</span>');
        ...
        if(jQuery('#agddata').val()==""){
            ...
            if(nd == undefined || nm == undefined || ny == undefined || ny.length > 4){
				jQuery('#erro_cv_e').slideUp().remove();
                jQuery('#'+local+'botcentral').prepend('<span id="erro_cv_e" style="padding:5px;border:1px red solid;color: red;font-size: 15px;">Data inválida!</span>');
                return false;
			}

			var hj = n_y + "" + (n_m<10 ? "0" : "" ) + n_m + "" + (n_d<10 ? "0" : "" ) + n_d;
			var ha = ny + "" + nm + "" + nd;
            ...
            if(errodata==1){
				jQuery('#erro_cv_e').slideUp().remove();
                jQuery('#'+local+'botcentral').prepend('<span id="erro_cv_e" style="padding:5px;border:1px red solid;color: red;font-size: 15px;">Data deve ser maior que hoje!</span>');
                return false;
            }
	...
	if(jQuery('#interesse').val()==""){
		...
		jQuery('#'+local+'botcentral').prepend('<span id="erro_cv_e" style="padding:5px;border:1px red solid;color: red;font-size: 15px;">Obrigatório selecionar interesse!</span>');
	...
	if(jQuery('#comoconheceu').val()==""){
		...
		jQuery('#'+local+'botcentral').prepend('<span id="erro_cv_e" style="padding:5px;border:1px red solid;color: red;font-size: 15px;">Obrigatório selecionar como conheceu!</span>');
	...
	if(jQuery('#nomec').val()==""){
		...
		jQuery('#'+local+'botcentral').prepend('<span id="erro_cv_e" style="padding:5px;border:1px red solid;color: red;font-size: 15px;">Obrigatório informar o nome do contratante!</span>');
	...
	if(jQuery('#cpf').val()==""){
		...
		jQuery('#'+local+'botcentral').prepend('<span id="erro_cv_e" style="padding:5px;border:1px red solid;color: red;font-size: 15px;">Obrigatório informar CPF do contratante!</span>');
    ...
    if(!validacpf(jQuery('#cpf').val())){
		...
		jQuery('#'+local+'botcentral').prepend('<span id="erro_cv_e" style="padding:5px;border:1px red solid;color: red;font-size: 15px;">Obrigatório informar um CPF válido!</span>');
   ...
   if(f==3){
       ...
       if(check=="false"){
		  ...
		  jQuery('#'+local+'botcentral').prepend('<span id="erro_cv_e" style="padding:5px;border:1px red solid;color: red;font-size: 15px;">Escolha um arquivo .doc, .pdf ou .rtf!</span>');
...
jQuery('#curriculo').change(function(){
    ...
    jQuery('.fileUpload.btn.btn-primary span').html("Arquivo Anexado (" + nome + ")");
...
/*function ajustagrid(elemento,plus){
    var h = 0;
    ...
    jQuery(elemento).each(function(){
        ...
        if(jQuery(this).height()>h){
			...
			jQuery(elemento).css("height", (plus+h)+"px");
        }
...
function ajustaatendimento(){
    var h = 0;

    jQuery(".atendimentosreais_class").each(function(){
    ...
    jQuery(".atendimentosreais_class").css("height", (20+h)+"px");
}*/